...
public class No2322 {
    ...
    class Solution {
        ...
        private Pair dfs(List<Integer>[] tree, int u, int prev, int[] subXors,
                                                Set<Integer>[] children) {
            for (final int v : tree[u]) {
                if (v == prev)
                    continue;
                final Pair pair = dfs(tree, v, u, subXors, children);
                final int vXor = pair.key;
                final Set<Integer> vChildren = pair.value;
                subXors[u] ^= vXor;
    ...
    private class Pair {
        ...
        public Pair(Integer key, Set<Integer> value) {
            this.key = key;
            this.value = value;
        }
    }
