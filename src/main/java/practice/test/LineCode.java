...
//        int howMuchNumber = Integer.parseInt(input);
//        int ans = 1;
//        String firstLetter = br.readLine();
//        for (int i = 1; i < howMuchNumber; i++) {
//            input = br.readLine();
//            if (input.charAt(0) == firstLetter.charAt(firstLetter.length() - 1)) {
//                ans++;
//            } else {
//                break;
//            }
...
//    }
    ...
    public static void main (String[] args) throws java.lang.Exception {
        ...
        input = br.readLine();
        int numberOfArchivers = Integer.parseInt(input);

        // calculate the minimum time to transfer the data
        double minTime = (double) dataSize / transferSpeed;

        // the data need three step to process: 
        // 1. compression
        // 2. transfer
        // 3. decompression
        for (int i = 0; i < numberOfArchivers; i++) {
            input = br.readLine();
            String[] archiverInfo = input.split(" ");
            // (both compression and extraction) speed in bytes per second
            int processingSpeed = Integer.parseInt(archiverInfo[0]);
            // the rate of compression achieved by the archiver
            int compressionRate = Integer.parseInt(archiverInfo[1]);
            
            // 1. compression
            double compressedSize = dataSize * (compressionRate / 100.0);
            double compressionTime = (double) dataSize / processingSpeed;
            // 2. transfer
            double transferTime = compressedSize / transferSpeed;
            // 3. decompression
            double decompressionTime = compressedSize / processingSpeed; 

            double totalTime = compressionTime + transferTime + decompressionTime;

            minTime = Math.min(minTime, totalTime);
        }

        System.out.println((int) Math.round(minTime));
    }
